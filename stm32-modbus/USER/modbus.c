#include "stm32f10x.h"
#include "modbus.h"
#include "Init_Config.h"
#include "Systick.h"
#include "uart1.h"

// Mudbus 功能码 
#define Display	0x03		// 数值显示
#define Modify  0x10		// 数据输入(多寄存器写入)显示
u32 crc16(u8 * puchMsg,u8 usDataLen);
// 保存接收与发送消息的数组
extern u8 ReceBuf[32];	            // 接收缓冲区
extern u8 SendBuf[32];	        	// 发送缓冲区
extern u8 sendnum;//发送数目
extern u8 recenum;//接收数目

extern u32 crcData;
extern u8 usDataLen;
u8 i;

//发送数据缓冲区
u16 XBuf[100]={0xf0,0xf1,0x02};

/*CRC校验表高位*/
unsigned char auchCRCHi[] = {

};

/* CRC校验表低位    */
unsigned char auchCRCLo[] = {

};



/*CRC校验函数，生成CRC*/
u32 crc16(u8 * pushMsg,u8 usDataLen)
{
	u8 uchCRCHi = 0xff;//高CRC字节初始化
	u8 uchCRCLo = 0xff;//低CRC字节初始化
	u8 uIndex;					//CRC循环中的索引
	while(usDataLen--)
	{
		uIndex = uchCRCHi^ *(pushMsg++);//计算CRC
		uchCRCHi = uchCRCLo^ auchCRCHi[uIndex];
		uchCRCLo= auchCRCLo[uIndex];
	}
	return (uchCRCHi << 8 | uchCRCLo);
}


void ParseRecieve8(void)	 //数据帧解析
{	
	u8 crcDataHi;
	u8 crcDataLo;
	u8 count;
	u8 sendny;
	u8 statusnum;
//	USART_SendData(USART1,ReceBuf[0]);
	if(ReceBuf[0] == 0x01)   //设备地址是否匹配
	{	
		crcData = crc16(ReceBuf,usDataLen);//从接收到的数据帧中获取CRC
		crcDataLo = crcData/256; //	 132 84
		crcDataHi = crcData%256; //	 10  0A
		if((crcDataHi == ReceBuf[7])&&(crcDataLo == ReceBuf[6]))//校验
		{
			switch(ReceBuf[1])
			{
				case 0x02:
					switch(ReceBuf[3]) //判断寄存器起始地址
					{
						case 0x00://aotomodbus
							sendnum = 5+(ReceBuf[4] * 256 + ReceBuf[5])/8;//回发的字节数
							sendny=(ReceBuf[4] * 256 + ReceBuf[5])%8;//余数
							if(sendny) sendnum+=1;
							usDataLen = sendnum - 2;
							SendBuf[0] = ReceBuf[0];//设备地址
							SendBuf[1] = ReceBuf[1];//功能号
							SendBuf[2] =sendnum-5;//返回数据的字节数
							statusnum=SendBuf[2];
							for(count = 3;statusnum>0;statusnum--)
							{
								SendBuf[count] = 0x01;
								count++;
							}
							crcData = crc16(SendBuf,usDataLen);
							crcDataLo = crcData/256; //低
							crcDataHi = crcData%256; //高
							SendBuf[sendnum - 1] = crcDataHi;//CRC高位
							SendBuf[sendnum - 2] = crcDataLo;//CRC低位
							for(i = 0;i < sendnum;i++)
							{	
								USART_SendData(USART1,SendBuf[i]);
								while(!(USART1->SR & USART_FLAG_TXE));
							}
							sendnum = 0;
							break;
					}
					break;
				case 0x04:
					switch(ReceBuf[3]) //判断寄存器起始地址
					{
						case 0x0b://aotomodbus
							sendnum = 5+(ReceBuf[4] * 256 + ReceBuf[5])*2;//回发的字节数
							usDataLen = sendnum - 2;
							SendBuf[0] = ReceBuf[0];//设备地址
							SendBuf[1] = ReceBuf[1];//功能号
							SendBuf[2] = sendnum - 5;//返回数据的字节数
							for(count = 3;ReceBuf[5] > 0;ReceBuf[5]--)
							{
								SendBuf[count] =  XBuf[count - 3]/256;//回发数据高8位
								SendBuf[count+1] = XBuf[count - 3]%256;//回发数据低8位
								count = count + 2;
							}
							crcData = crc16(SendBuf,usDataLen);
							crcDataLo = crcData/256; //低
							crcDataHi = crcData%256; //高
							SendBuf[sendnum - 1] = crcDataHi;//CRC高位
							SendBuf[sendnum - 2] = crcDataLo;//CRC低位
							for(i = 0;i < sendnum;i++)
							{	
								USART_SendData(USART1,SendBuf[i]);
								while(!(USART1->SR & USART_FLAG_TXE));
							}
							sendnum = 0;
							break;
					}
					break;
				case 0x03:
					switch(ReceBuf[3]) //判断寄存器起始地址
					{
						case 0x00://从第一个显示窗口开始
							sendnum = 5+(ReceBuf[4] * 256 + ReceBuf[5])*2;//回发的字节数
							usDataLen = sendnum - 2;
							SendBuf[0] = ReceBuf[0];//设备地址
							SendBuf[1] = ReceBuf[1];//功能号
							SendBuf[2] = sendnum - 5;//返回数据的字节数
							for(count = 3;ReceBuf[5] > 0;ReceBuf[5]--)
							{
								SendBuf[count] =  XBuf[count - 3]/256;//回发数据高8位
								SendBuf[count+1] = XBuf[count - 3]%256;//回发数据低8位
								count = count + 2;
							}
							crcData = crc16(SendBuf,usDataLen);
							crcDataLo = crcData/256; //低
							crcDataHi = crcData%256; //高
							SendBuf[sendnum - 1] = crcDataHi;//CRC高位
							SendBuf[sendnum - 2] = crcDataLo;//CRC低位
							for(i = 0;i < sendnum;i++)
							{	
								USART_SendData(USART1,SendBuf[i]);
								while(!(USART1->SR & USART_FLAG_TXE));
							}
							sendnum = 0;
							break;
					}
					break;
			}
		}
	}
}
